...
/** Map a public domain to this service's port, with optional certbot HTTPS. */
export default function DomainsTab({ service, onChange }) {
  ...
  const add = async () => {
    ...
    try {
      await api.addDomain(service.id, { host, port: Number(port), https, email });
  ...
  return (
    <div className="space-y-5 max-w-3xl">
      <div className="card p-6">
        ...
        <p className="text-xs text-muted mb-4">Generates an nginx reverse-proxy and (optionally) a Let's Encrypt certificate.</p>
        ...
        <div className="flex justify-end mt-4">
          <button onClick={add} disabled={busy || !host || !port} className="btn-brand">
          ...
          {(service.domains || []).map((d) => (
            <div key={d.host} className="flex items-center justify-between bg-bg-input border border-line rounded-lg px-3.5 py-2.5">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-xs text-muted">→ :{d.port}</span>
